...
#Solve steady 2D advection-Diffusion eqn with given steady double gyre velocity profile using perturbation method:


def geo_train(device,x_in,y_in,xb,yb,cb,xb_N_up,yb_N_up, xb_N_down,yb_N_down, batchsize,learning_rate,epochs,path,Flag_batch,Diff,Flag_BC_exact,Lambda_BC  ):
	if (Flag_batch):
	...
	def Velocity(x_input,y_input):
		...
		A = -0.1
		pi = math.pi
		Vel_x = -pi*A*torch.sin(pi*(a_t * torch.pow(2.*x_input, 2.0)+ b_t* 2.*x_input))*torch.cos(pi*y_input)
		Vel_y = pi*A*(2.0*a_t *2.*x_input+2.*b_t)*torch.cos(pi*(a_t * torch.pow(2.*x_input , 2.0)+ b_t *2.*x_input))*torch.sin(pi*y_input)
		return Vel_x,Vel_y
	...
	def criterion_inner(x,y):
		...
		u,v = Velocity(x,y*inf_scale * math.sqrt(Diff) )
	...
	def criterion_outer(x,y):
		...
		loss_1 = u *c_x + v*c_y - Diff * (c_xx + c_yy)
	...
	def Loss_BC(xb,yb,xb_N_up,yb_N_up ,xb_N_down,yb_N_down,cb):
	...
	if(Flag_batch):# This one uses dataloader
		for epoch in range(epochs):
			...
			if (epoch % 5000 == 0):
				torch.save(net2_outer.state_dict(),path+"final_gyre_outer" + ".pt")
				torch.save(net2_inner.state_dict(),path+"final_gyre_inner" + ".pt")
...
batchsize = 256 #128  #Total number of batches 
...
if(1): #Dirichlet BC left and right
 xb = np.concatenate((xleft, xright), 0) #np.concatenate((xleft, xright, xup, xdown), 0)
 yb = np.concatenate((yleft, yright), 0) #np.concatenate((yleft, yright, yup, ydown), 0)
 cb = np.concatenate((cleft, cright), 0)# np.concatenate((cleft, cright, cup, cdown), 0)
...
geo_train(device,x,y,xb,yb,cb,xb_N_up,yb_N_up, xb_N_down,yb_N_down, batchsize,learning_rate,epochs,path,Flag_batch,Diff,Flag_BC_exact,Lambda_BC  )
#tic = time.time()
